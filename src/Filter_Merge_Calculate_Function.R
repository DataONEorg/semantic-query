...
filter_merge_calculate_function <- function(gtFileLocation, outputFileLocation, homePath){
  ...
  #gtFileLocation <- '~/dataone/gitcheckout/semantic-query/lib/ground_truth/test_corpus_f_groundtruth_carbon_flux_queries.csv'
  ...
  #outputFileLocation <-'~/dataone/gitcheckout/semantic-query/results/Resultset_Summary_2016-03-15 11:35:14_.csv'
  ...
  outputFilePath <- paste0(homePath, "semantic-query/results/") 
  ...
  #Lines 68 to 77 obtains the valid value (i.e. non-zeor) for "Run_ID" from the "filtered_merged_result" data frame.
  run_id_col <- unique(filtered_merged_result[c("Run_ID")])

  for(i in 1:(nrow(run_id_col))){
  ...
  finalOutput <- paste0(homePath, "semantic-query/results/Prec_Recall_Results.txt")
  #print(finalOutput)
